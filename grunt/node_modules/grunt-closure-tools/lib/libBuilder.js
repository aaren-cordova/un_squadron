...
builder.createCommand = function createCommand ( options, fileObj ) {
  ...
  if (inputs && inputs.length) {
    args.push(cHelpers.makeParam( inputs, '-i', false, true));
  }
  // check for namespaces
  if (options.namespaces && options.namespaces.length) {
    args.push(cHelpers.makeParam( options.namespaces, '-n' ));
  }
  // append root
  var allRoots = gruntMod.file.expand( fileObj.src );
  args.push(cHelpers.makeParam( allRoots, '--root=', true, true));
  ...
  if (dest && dest.length) {
    args.push(' --output_file=' + gruntMod.template.process(dest));
  }
  ...
  // ---
  if (canCompile) {
    args.push(' --compiler_jar=' + options.compilerFile);
    ...
    for( var directive in compileOpts ) {
      ...
      if ( Array.isArray( compileOpts[directive] ) ) {
        ...
        for (var i = 0, len = compileOpts[directive].length; i < len; i++) {
          args.push(' --compiler_flags="--' + directive + '=' + compileOpts[directive][i] + '"');
        }
      }
      else if (null === compileOpts[directive]) {

        // value of directive is null, not required
        args.push(' --compiler_flags="--' + directive + '"');
      } else {

        // value of directive is a string
        args.push(' --compiler_flags="--' + directive + '=' + compileOpts[directive] + '"');
      }
